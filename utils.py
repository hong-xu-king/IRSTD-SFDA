...
def random_crop_nyh(img, mask, body, detail, patch_size, pos_prob=None):
    h, w = img.shape
    if min(h, w) < patch_size:
        img = np.pad(img, ((0, max(h, patch_size) - h), (0, max(w, patch_size) - w)), mode='constant')
        mask = np.pad(mask, ((0, max(h, patch_size) - h), (0, max(w, patch_size) - w)), mode='constant')
        body = np.pad(body, ((0, max(h, patch_size) - h), (0, max(w, patch_size) - w)), mode='constant')
        detail = np.pad(detail, ((0, max(h, patch_size) - h), (0, max(w, patch_size) - w)), mode='constant')
        h, w = img.shape
    ...
    while 1:
        ...
        img_patch = img[h_start:h_end, w_start:w_end]
        mask_patch = mask[h_start:h_end, w_start:w_end]
        body_patch = body[h_start:h_end, w_start:w_end]
        detail_patch = detail[h_start:h_end, w_start:w_end]
        if pos_prob == None or random.random() > pos_prob:
            break
        elif mask_patch.sum() > 0:
            break
...
def get_optimizer1(net, optimizer_name, scheduler_name, optimizer_settings, scheduler_settings):
    if optimizer_name == 'Adam':
        optimizer = torch.optim.Adam(net, lr=optimizer_settings['lr'])
    elif optimizer_name == 'Adagrad':
        optimizer = torch.optim.Adagrad(net, lr=optimizer_settings['lr'])
    elif optimizer_name == 'SGD':
        optimizer = torch.optim.SGD(net, lr=optimizer_settings['lr'])
...
def calculate_threshold_and_visualize(output_tensor):
    # 将 GPU 上的 tensor 转移到 CPU 并转换为 NumPy 数组
    output_array = output_tensor.detach().cpu().numpy().ravel()

    # 确保数据在0-1范围内，如不在范围内则进行归一化
    output_array = (output_array - output_array.min()) / (output_array.max() - output_array.min())

    # 使用双峰高斯混合模型拟合数据，设置初始均值
    initial_means = np.array([[0.05], [0.95]])  # 将均值初始设为0.1和0.9
    gmm = GaussianMixture(n_components=2, means_init=initial_means, random_state=0)
    gmm.fit(output_array.reshape(-1, 1))

    # 获取两个高斯分布的均值和方差
    means = gmm.means_.flatten()
    variances = gmm.covariances_.flatten()

    # 根据均值排序，确保较小的均值为背景，较大的为前景
    idx = np.argsort(means)
    mean1, mean2 = means[idx[0]], means[idx[1]]
    var1, var2 = variances[idx[0]], variances[idx[1]]

    # 计算两个高斯分布的交点作为分割阈值
    # threshold = (var1 * mean2 - var2 * mean1) / (var1 - var2)
    threshold = abs(mean2-mean1)/2
    # # 可视化结果
    # plt.figure(figsize=(10, 6))
    #
    # # 绘制模型输出的直方图
    # plt.hist(output_array, bins=50, density=True, alpha=0.6, color='gray', label="Model Output Distribution")
    #
    # # 绘制高斯分布拟合曲线
    # x = np.linspace(output_array.min(), output_array.max(), 1000)
    # g1 = norm.pdf(x, mean1, np.sqrt(var1))
    # g2 = norm.pdf(x, mean2, np.sqrt(var2))
    # plt.plot(x, g1, color='blue', label=f'Gaussian 1 (mean={mean1:.2f})')
    # plt.plot(x, g2, color='green', label=f'Gaussian 2 (mean={mean2:.2f})')
    #
    # # 绘制分割阈值线
    ...
    #
    # # 设置标题和标签
    # plt.title("GMM-based Thresholding on Model Output")
