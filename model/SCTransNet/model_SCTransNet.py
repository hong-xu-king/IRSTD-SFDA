...
import zipimport
import sys
# whl_path = 'thop-0.1.1.post2209072238-py3-none-any.whl'
# zip_importer = zipimport.zipimporter(whl_path)
# sys.path_hooks.append(zip_importer)
#
# # 将 .whl 文件添加到 sys.path
...
class Res_block(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        out = self.relu(out)
        outv2 = self.conv2(out)
        outv2 = self.bn2(outv2)
        outv2 = self.relu(outv2)
        out = self.conv3(outv2)
        out = self.bn3(out) + outv2
...
class ResidualUpSample(nn.Module):
    def __init__(self, in_channels, out_channles ,bias=False):
        super(ResidualUpSample, self).__init__()
        self.top = nn.Sequential(nn.Conv2d(in_channels, in_channels, 1, stride=1, padding=0, bias=bias),
                                nn.PReLU(),
                                nn.ConvTranspose2d(in_channels, in_channels, 3, stride=2, padding=1, output_padding=1,bias=bias),
                                nn.PReLU(),
                                nn.Conv2d(in_channels, out_channles, 1, stride=1, padding=0, bias=bias))
...
class SCTransNet(nn.Module):
    def __init__(self, config=get_CTranS_config(), n_channels=1, n_classes=1, img_size=256, vis=False, mode='test', deepsuper=True):
        super().__init__()
        self.vis = vis
        ...
        self.resup4 = ResidualUpSample(256,128)
        self.resup3 = ResidualUpSample(128,64)
        self.resup2 = ResidualUpSample(64,32)
    ...
    def forward(self, x):
        ...
        d4_res = self.resup4(d5)
        d4 = d4 + d4_res
        d3 = self.up_decoder3(d4, x3)
        d3_res = self.resup3(d4)
        d3 = d3 + d3_res
        d2 = self.up_decoder2(d3, x2)
        d2_res = self.resup2(d3)
        d2 = d2 + d2_res
