...
class ESDNET(nn.Module):
    def __init__(self,
        ...
        if self.deep_supervision:
            self.conv_3 = nn.Conv2d(de_feature_num//2*3, 1, kernel_size=1 )
            self.conv_1 = nn.Conv2d(de_feature_num//2, 1, kernel_size=1)
            self.conv_2 = nn.Conv2d(de_feature_num//2*2, 1, kernel_size=1)
        else:
            self.conv_f = nn.Conv2d(de_feature_num // 2, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        if self.deep_supervision:
            output1 = self.conv_1(out_1)
            output2 = self.conv_2(torch.cat([self.up(out_2),out_1],1))
            # output3 = out_3.sigmoid()
            output3 = self.conv_3(torch.cat([self.up_4(out_3),self.up(out_2),out_1],1))
            if self.mode == 'train':
                return [output1, output2, output3]
            else:
                return output1
        else:
            output = self.conv_f(out_1)
            return output
        # return out_1
...
class Decoder_Level(nn.Module):
    def __init__(self, feature_num, inter_num, sam_number):
        ...
        for _ in range(sam_number):
            sam_block = SAM(in_channel=feature_num, d_list=(1, 2, 3, 2, 1), inter_num=inter_num)
            self.sam_blocks.append(sam_block)
        self.conv = conv(in_channel=feature_num, out_channel=feature_num*2, kernel_size=3, padding=1)
