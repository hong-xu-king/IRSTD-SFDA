...
parser.add_argument("--model_names", default=['DNANet'], type=list,
                    help="model_name: 'ACM', 'ALCNet', 'DNANet', 'ISNet', 'UIUNet', 'RDIAN', 'ISTDU-Net', 'U-Net', 'NyhNet'")
parser.add_argument("--dataset_names", default=['NUAA-SIRST'], type=list,
                    help="dataset_name: 'NUAA-SIRST', 'czzzz-SIRST', 'IRSTD-1K', 'SIRST3', 'NUDT-SIRST-Sea', 'IRDST-real.")
...
parser.add_argument("--optimizer_settings", default={'lr': 1e-4}, type=dict, help="optimizer settings")
...
def train():
    ...
    if opt.optimizer_name == 'Adam':
        opt.optimizer_settings = {'lr': 1e-4}
        opt.scheduler_name = 'CosineAnnealingLR'
        opt.scheduler_settings = {'epochs': 2000, 'min_lr': 1e-5}
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        for idx_iter, (img, gt_mask) in enumerate(train_loader):
            ...
            bce_loss = F.binary_cross_entropy_with_logits(pred, gt_mask)
            miou_loss = iou_loss(pred, gt_mask)
            loss = miou_loss + bce_loss
